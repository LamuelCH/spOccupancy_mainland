...
write.csv(df_filtered, "input/df_multiSeason.csv", row.names = FALSE)
...
y <- array(NA, dim = c(N, J, K, R))
dimnames(y)[[1]] <- genus.codes
dimnames(y)[[2]] <- as.character(plot.codes)
dimnames(y)[[3]] <- as.character(year.codes)
dimnames(y)[[4]] <- as.character(replicate.codes)

# Create array for effort data
effort.matrix <- array(NA, dim = c(J, K, R),
                       dimnames = list(as.character(plot.codes), 
                                       as.character(year.codes), 
...
for (j in 1:J) {
  ...
  for (k in 1:K) {
    for (r in 1:R) {
      ...
      # Check if this site-year-replicate combination was sampled
      effort_data <- effort.long %>%
        filter(plotID == site, Year == year, replicate == rep)
      ...
      if (nrow(effort_data) > 0) {
        ...
        for (i in 1:N) {
          ...
          species_data <- y.long %>%
            filter(plotID == site, Year == year, replicate == rep, Genus == genus)
          
          if (nrow(species_data) > 0) {
            y[i, j, k, r] <- species_data$occ[1]
          } else {
            y[i, j, k, r] <- 0  # Species not detected
          }
        }
      }
...
for (j in 1:J) {
  for (k in 1:K) {
    for (r in 1:R) {
      ...
      # Get effort for this site, year, and replicate combination
      val <- effort.long %>%
        filter(plotID == site, Year == year, replicate == rep) %>%
        pull(effort_days)
...
project_summary <- df_filtered %>%
  group_by(plotID, Year, replicate, ProjectID) %>%
  summarize(
...
project.matrix <- array(NA, dim = c(J, K, R),
                        dimnames = list(as.character(plot.codes), 
                                        as.character(year.codes), 
                                        as.character(replicate.codes)))
...
for (i in 1:nrow(project_summary)) {
  ...
  k <- which(year.codes == row$Year)
  r <- which(replicate.codes == row$replicate)
  
  project.matrix[j, k, r] <- row$ProjectID
}
...
# Get year codes and plot codes from the data
year_codes <- sort(unique(df_filtered$Year))
plot_codes <- sort(unique(df_filtered$plotID))
J <- length(plot_codes)
K <- length(year_codes)

# Create years matrix where each row contains all years
years_matrix <- matrix(as.integer(rep(year_codes, J)), 
                       nrow = J, 
                       ncol = K, 
                       byrow = TRUE)

# Create site.effect vector
site.effect <- as.integer(plot_codes)
...
env_vars <- setdiff(names(occ.covs), c("years", "site.effect"))
...
if(length(rows_with_na) > 0) {
  ...
  # Clean environmental covariates
  for (var in env_vars) {
    occ.covs[[var]] <- occ.covs[[var]][-rows_with_na]
  }
  
  # Clean years matrix and site.effect
  occ.covs$years <- occ.covs$years[-rows_with_na, , drop = FALSE]
  occ.covs$site.effect <- occ.covs$site.effect[-rows_with_na]
...
cat("Number of sites after cleaning:", length(occ.covs$site.effect), "\n")
...
validate_occupancy_data <- function(data) {
  ...
  cat("Expected: [species, sites, years, replicates]\n")
  
  # Check NA patterns
  na_pattern <- apply(data$y, c(2,3,4), function(x) all(is.na(x)))
  cat("Sites with all NAs:", sum(apply(na_pattern, 1, all)), "\n")
  cat("Years with all NAs:", sum(apply(na_pattern, 2, all)), "\n")
  cat("Replicates with all NAs:", sum(apply(na_pattern, 3, all)), "\n")
  ...
  cat("Years match:", y_dims[3] == ncol(data$occ.covs$years), "\n")
  cat("Effort aligns:", all(effort_dims == y_dims[2:4]), "\n")
...
cat("\nSaving data...\n")
save(data, metadata, file = "input/data_multiSeason.RData")
cat("\nData preparation complete!\n")
cat("Output saved to: input/data_multiSeason.RData\n")
...
# Test run
occ.formula = ~ scale(years) + I(scale(years)^2) + scale(bio5) + I(scale(bio5)^2) + scale(bio6) + I(scale(bio6)^2) + 
  scale(bio12) + I(scale(bio12)^2) + scale(bio15) + I(scale(bio15)^2) + I(scale(dem)^2) + scale(eastness) + scale(northness) +
  scale(tpi) + I(scale(tpi)^2) + scale(tri) + I(scale(tri)^2) +
  (1 | years) + (1 | site.effect)

det.formula = ~ scale(effort) + (1|project)


test.sfMsPGOcc <- stMsPGOcc(occ.formula = occ.formula,
